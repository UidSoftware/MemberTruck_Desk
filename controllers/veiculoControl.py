# Arquivo: controllers/veiculoControl.py
...
class VeiculoControl(CRUDController):
    """
    ...
    def criar(self, *args, **kwargs):
        data = {
            "nomeVeic": self.view.ids.veiculo_nomeVeic.text,
            "anoVeic": self.view.ids.veiculo_anoVeic.text,
            "placaVeic": self.view.ids.veiculo_placaVeic.text,
        }
        if not all(data.values()):
            self.show_snackbar("Erro: Todos os campos são obrigatórios.")
            return
    ...
    def _handle_response_criar(self, future):
        try:
            ...
            if response.status_code == 201:
                self.show_snackbar("Veículo criado com sucesso!")
                self.clear_form()
            else:
    ...
    def _handle_response_ler(self, future):
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.view.ids.veiculo_nomeVeic.text = data.get("nomeVeic", "")
                self.view.ids.veiculo_anoVeic.text = data.get("anoVeic", "")
                self.view.ids.veiculo_placaVeic.text = data.get("placaVeic", "")
                self.show_snackbar("Veículo carregado.")
            else:
    ...
    def _handle_response_atualizar(self, future):
        try:
            response = future.result()
            if response.status_code == 200:
                self.show_snackbar("Veículo atualizado com sucesso!")
                self.clear_form()
            else:
    ...
    def _handle_response_deletar(self, future):
        try:
            ...
            if response.status_code == 204:
                self.show_snackbar("Veículo excluído com sucesso!")
                self.clear_form()
            else:
    ...
    def clear_form(self):
        self.view.ids.veiculo_nomeVeic.text = ""
        self.view.ids.veiculo_anoVeic.text = ""
        self.view.ids.veiculo_placaVeic.text = ""
